...
class LoginPage(TermsAndConditionsPopup, SubscriptionHasExpitredPopup, ErrorPopup):
    ...
    def login(self):
        try:
            ...
            cond1 = self.is_element_present(ErrorPopup.FRAME)
            cond2 = self.is_element_present(SubscriptionHasExpitredPopup.FRAME)
            if cond1:
                ...
                pass
            self.wait_for_element_present(RibbonBar._RIBBON_BAR)
            self.wait_for_elements_present(MainPage._PANEL)
    ...
    def check_login_page_loaded(self):
        cond1 = self.is_element_present(LoginPage.BUTTON_SIGN_IN)
        cond2 = self.is_element_present(LoginPage.FIELD_USERNAME)
        cond3 = self.is_element_present(LoginPage.FIELD_PASSWORD)
