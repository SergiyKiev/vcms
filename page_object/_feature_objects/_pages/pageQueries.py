...
class QueriesPage(BaseActions):

    PAGE_HEADER = "//span[text()='Queries']/ancestor::div[@class='Panel-Control'][contains(@style,'85px')]"
    ...
    def check_page_is_present(self):
        cond = self._is_element_present(QueriesPage.PAGE_HEADER)
        return True if cond else False

    def click_icon_help(self):
        self._click_icon_help(QueriesPage.PAGE_HEADER)
    ...
    def click_icon_refresh(self):
        self._click_icon_refresh(QueriesPage.PAGE_HEADER)

    def click_icon_search(self):
        self._click_icon_search(QueriesPage.PAGE_HEADER)
    ...
    def check_query_is_present(self, name):
        cond = self._is_element_present(QueriesPage.TABLE_ROW + "/*//span[text()='" + str(name) + "']/ancestor::tr")
        return True if cond else False
    ...
    def select_query_in_table(self, *name):
        self._wait_for_element_present(QueriesPage.TABLE_ROW)
        row = QueriesPage.TABLE_ROW + "/*//span[text()='" + str(*name) + "']/ancestor::tr"
