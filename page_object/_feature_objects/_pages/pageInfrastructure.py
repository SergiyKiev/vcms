...
class InfrastructurePage(BaseActions):

    PAGE_HEADER = "//span[text()='Infrastructure']/ancestor::div[@class='Panel-Control'][contains(@style,'85px')]"
    ...
    def check_page_is_present(self):
        cond = self._is_element_present(InfrastructurePage.PAGE_HEADER)
        return True if cond else False

    def click_icon_refresh(self):
        self._click_icon_refresh(InfrastructurePage.PAGE_HEADER)

    def click_icon_search(self):
        self._click_icon_search(InfrastructurePage.PAGE_HEADER)

    def enter_text_into_search_text_field(self, text = None):
        self._find_element(InfrastructurePage.SEARCH_FIELD).send_keys(text)
    ...
    def click_icon_help(self):
        self._click_icon_help(InfrastructurePage.PAGE_HEADER)
