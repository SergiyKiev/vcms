...
class DynamicallyManagedPage(BaseActions):
    ...
    def check_page_is_present(self):
        cond = self._is_element_present(DynamicallyManagedPage.PAGE_HEADER)
        return True if cond else False

    def click_icon_refresh(self):
        self._click_icon_refresh(DynamicallyManagedPage.PAGE_HEADER)

    def click_icon_search(self):
        self._click_icon_search(DynamicallyManagedPage.PAGE_HEADER)

    def enter_text_into_search_text_field(self, text = None):
        self._find_element(DynamicallyManagedPage.SEARCH_FIELD).send_keys(text)

    def check_device_is_present(self, name):
        row = DynamicallyManagedPage.TABLE_ROW + "/*//span[text()='" + name + "']/ancestor::tr"
        cond = self._is_element_present(row)
        return True if cond else False

    def click_icon_help(self):
        self._click_icon_help(DynamicallyManagedPage.PAGE_HEADER)
