...
class ExcludedDevicesPage(BaseActions):

    PAGE_HEADER = "//span[text()='Excluded Devices']/ancestor::div[@class='Panel-Control'][contains(@style,'85px')]"
    ...
    def check_page_is_present(self):
        cond = self._is_element_present(ExcludedDevicesPage.PAGE_HEADER)
        return True if cond else False

    def click_icon_refresh(self):
        self._click_icon_refresh(ExcludedDevicesPage.PAGE_HEADER)

    def click_icon_search(self):
        self._click_icon_search(ExcludedDevicesPage.PAGE_HEADER)

    def enter_text_into_search_text_field(self, text = None):
        self._find_element(ExcludedDevicesPage.SEARCH_FIELD).send_keys(text)

    def check_device_is_present(self, name):
        row = ExcludedDevicesPage.TABLE_ROW + "/*//span[text()='" + name + "']/ancestor::tr"
        cond = self._is_element_present(row)
        return True if cond else False

    def click_icon_help(self):
        self._click_icon_help(ExcludedDevicesPage.PAGE_HEADER)
